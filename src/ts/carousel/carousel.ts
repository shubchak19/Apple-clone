import { Promo, promoData } from "./promoData.js";
...
function createItem({ image, phoneImage, name, type, data }: Promo) {
  return `
    <div class="promo">
...
export function initializeCarousel() {
  ...
  promoData.forEach((itemData, index) => {
    ...
    allIndicators += `<li id=${itemData.id} class="indicator-wrapper ${
...
export function goToItem(index: number) {
  if (!carousel || !indicatorContainer || !carouselContainer) return;
...
export function handleResize() {
  ...
  resizeTimeoutId = setTimeout(() => {
    if (carousel) carousel.style.transition = "transform 0.5s ease-in-out";
  }, 100);
}
